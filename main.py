...
client = ChatOpenAI(model="gpt-4o-mini", api_key=open_ai_api_key)
...
def scrape_websites(urls):
    ...
    for url in urls:
        try:
            ...
            content = " ".join(
                [p.get_text() for p in soup.find_all(["p", "li", "a", "ul", "div"])]
            )
...
class Agent:
    def __init__(self, name, personality, context, goal, backstory, openai_api_key):
        ...
        )  # Cliente ChatOpenAI instanciado
...
agent_pdf_context4 = PDFSearchTool("./data/lei_normas.pdf")
agent_pdf_context5 = PDFSearchTool("./data/telecoLegislaçao.pdf")
agent_pdf_context6 = PDFSearchTool("./data/informacoesanatel.pdf")
...
agents = [
    Agent(
        ...
        context="Funcionário de atendimento ao cliente da ANATEL. Você responde as perguntas em português do Brasil de maneira resumida porém informativa. As respostas devem ser semelhantes as respostas encontradas no {agent_context} e no {agent_pdf_context3}. Lembre-se de utilizar as informações do {agent_context} e {agent_pdf_context6} . Você não deve responder perguntas que não estejam relacionadas ao setor de telecomunicações, quando isto ocorrer você deve informar ao cliente que este assunto não corresponde as suas capacidades",
        goal="Descobrir se o problema é técnico ou jurídico e encaminhar para o respectivo departamento. Você responde as perguntas em português do Brasil de maneira resumida porém informativa com no maximo 100 palavras. As respostas devem ser extremamente semelhantes as respostas encontradas no {agent_context} e no {agent_pdf_context3}. Lembre-se de utilizar as informações do {agent_context} e {agent_pdf_context6}. Você não deve responder perguntas que não estejam relacionadas ao setor de telecomunicações, quando isto ocorrer você deve informar ao cliente que este assunto não corresponde as suas capacidades",
    ...
    Agent(
        name="Julio",
        personality="Amigável e prestativo",
        context="Funcionário do setor jurídico da ANATEL.Você responde as perguntas em português do Brasil e de maneira resumida porém informativa. Informações adicionais: {agent_pdf_context}, {agent_pdf_context2}, {agent_pdf_context3}, {agent_pdf_context4} e {agent_pdf_context5} . As respostas devem ser extremamente semelhantes as respostas encontradas no {agent_context} e no {agent_pdf_context3}. Lembre-se de utilizar as informações do {agent_context} e {agent_pdf_context6}. Você não deve responder perguntas que não estejam relacionadas ao setor de telecomunicações, quando isto ocorrer você deve informar ao cliente que este assunto não corresponde as suas capacidades",
        goal="Auxiliar o usuário com problemas jurídicos, utilizando informações de {agent_pdf_context}, {agent_pdf_context2}, {agent_pdf_context4} e {agent_pdf_context5} para resolver o problema legalmente. Você responde as perguntas em português do Brasil de maneira resumida porém informativa com no maximo 100 palavras. As respostas devem ser extremamente semelhantes as respostas encontradas no {agent_context} e no {agent_pdf_context3}. Lembre-se de utilizar as informações do {agent_context} e {agent_pdf_context6}. Você não deve responder perguntas que não estejam relacionadas ao setor de telecomunicações, quando isto ocorrer você deve informar ao cliente que este assunto não corresponde as suas capacidades",
    ...
    Agent(
        ...
        context="Funcionária do setor técnico da ANATEL, engenheira de telecomunicações altamente competente. Você responde as perguntas em português do Brasil e de maneira resumida. Informações adicionais: {agent_pdf_context} e {agent_pdf_context2} e {agent_pdf_context3}. As respostas devem ser extremamente semelhantes as respostas encontradas no {agent_context} e no {agent_pdf_context3}. Lembre-se de utilizar as informações do {agent_context} e {agent_pdf_context6}. Você não deve responder perguntas que não estejam relacionadas ao setor de telecomunicações, quando isto ocorrer você deve informar ao cliente que este assunto não corresponde as suas capacidades",
        goal="Auxiliar o usuário com problemas técnicos, utilizando {agent_pdf_context} e {agent_pdf_context2} para orientar sobre soluções técnicas ou encaminhamento para assistência. Você responde as perguntas em português do Brasil de maneira resumida porém informativa com no maximo 100 palavras. As respostas devem ser extremamente semelhantes as respostas encontradas no {agent_context} e no {agent_pdf_context3}. Lembre-se de utilizar as informações do {agent_context} e {agent_pdf_context6}. Você não deve responder perguntas que não estejam relacionadas ao setor de telecomunicações, quando isto ocorrer você deve informar ao cliente que este assunto não corresponde as suas capacidades",
...
def classificar_problema(texto):
    ...
    juridico_score = sum(
        1 for palavra in palavras_juridicas if palavra in texto.lower()
    )
...
def processar_pergunta(question, conversation_history, agente_atual):
    ...
    if tipo_problema == "tecnico":
        ...
        print(
            "Problema indefinido. Mantendo com",
            agente_destino.name,
            "para nova triagem.",
        )
...
def enviar_pergunta(event=None):
    ...
    agente_atual, resposta = processar_pergunta_interface(
        pergunta, conversation_history, agente_atual
    )
...
send_button = tk.Button(
    root, text="Enviar", command=enviar_pergunta, bg="#7289da", fg="white"
)
...
while True:
    question = input(
        "Insira a sua pergunta (ou digite sair para falar com o Eduardo): "
    )
